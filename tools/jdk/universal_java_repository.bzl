...
java_runtime(
    name = "jdk",
    srcs = glob(["bin/**", "include/**", "lib/**", "modules"]) + [
        "@{linux_repo}//:jdk",
        "@{macos_repo}//:jdk",
    ],
)
...
def _universal_java_repo_impl(ctx):
    ...
    for dir in _MERGED_DIRS:
        ctx.execute(["/bin/bash", "-c", """
            set -e
            mkdir include
            for x in $(ls ../{linux_repo}/include); do
                ln -s ../../{linux_repo}/include/$x include/$x
            done
            for x in $(ls ../{macos_repo}/include); do
                ln -s ../../{macos_repo}/include/$x include/$x
            done""".format(linux_repo = ctx.attr.linux_repo, macos_repo = ctx.attr.macos_repo)])
